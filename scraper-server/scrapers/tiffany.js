...
function parseTile(text){
    ...
    let banned = /^(previous|next|soldes|nouveau|new|nouveautés?|voir tout|découvrir|avis|ignorer.*|prix réduit de|à|\+\d+|-?\d+%|\d+\s*matéri\w*)$/i;
...
const EXTRA_CATEGORIES = [
    "https://www.tiffany.fr/jewelry/necklaces-pendants/",
    "https://www.tiffany.fr/jewelry/earrings/",
    "https://www.tiffany.fr/jewelry/bracelets/",
    "https://www.tiffany.fr/jewelry/brooches/"
];
...
async function scrapeOneCategory(page, url){
    ...
    // clique "Plus de résultats" (Algolia InfiniteHits) jusqu'à épuisement
    for(let i=0;i<30;i++){

        let clicked = await page.evaluate(()=>{
            let btn = document.querySelector(".ais-InfiniteHits-loadMore");
...
async function scrapeTiffany(url, brand, category){
